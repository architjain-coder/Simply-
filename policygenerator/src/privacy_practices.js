...
function getFilePath(relativePath){
    ...
    */
    if(getattr(sys, 'frozen', False)){
        ...
        return os.path.join(application_path, relativePath.slice(3))
    }
    else{
...
function retrieve_privacy_practice_data(){
    /*
    ...
    */
    ...
    evidence = open(pp, 'r', encoding="utf-8")
    ...
    for(practice in Practices.practices){
        ...
        for(language in ["SWIFT", "OBJECTIVE-C"]){
            ...
            practice_data[language]["PLIST"] = privacy_practices["PLIST"][practice_string]
            practice_data[language]["ENTITLEMENT"] = privacy_practices["ENTITLEMENT"][practice_string]
            practice_data[language]["FRAMEWORK"] = privacy_practices["FRAMEWORK"][language][practice_string]
            practice_data[language]["CLASS"] = privacy_practices["CLASS"][language][practice_string]
            practice_data[language]["AUTHORIZATION_METHOD"] = privacy_practices["AUTHORIZATION_METHOD"][language][practice_string]
            practice_data[language]["ADDITIONAL_EVIDENCE"] = privacy_practices["ADDITIONAL_EVIDENCE"][language][practice_string]
        }
...
function search_root_dir(directory){
    /*
    ...
    */
    ...
    for(root, dirs, files in os.walk(directory)){
        for(f in files){
            ...
            if("Pods" in root || "Carthage" in root)
                pass
            else
...
function get_pod_loc(directory){
    /*
...
function get_cart_loc(directory){
    /*
...
function grab_third_party_files(sdks){
    /*
...
function locate_entitlements_file(app_dir){
    /*
